...
async function githubFetch<T>(path: string): Promise<T> {
  ...
  const res = await fetch(url, {
    headers: {
      ...
      'User-Agent': 'gha-optimizer-mcp/0.1.0',
    },
  });
...
const server = new McpServer({ name: 'gha-optimizer-mcp', version: '0.1.0' });
...
server.registerTool(
  ...
  async ({ owner, repo, workflow_id, count = 10 }) => {
    ...
    for (const run of runs) {
      const jobsData = await githubFetch<JobsResponse>(
    ...
    const topStepStats: Record<string, Array<{ step: string; avg: string; p95: string; raw_avg_seconds: number }>> = {};
    ...
    const overallRunDurations = runSummaries.map((r) => r.total_seconds).sort((a, b) => a - b);
    ...
    const report: Record<string, unknown> = {
      ...
      overall_timing: {
        avg_total: fmtSeconds(avg(overallRunDurations)),
        min_total: fmtSeconds(overallRunDurations[0] ?? 0),
        max_total: fmtSeconds(overallRunDurations[overallRunDurations.length - 1] ?? 0),
        p95_total: fmtSeconds(percentile(overallRunDurations, 95)),
      },
    ...
    const lines: string[] = [
      ...
      '',
      `Overall run timing:`,
      `  avg: ${report.overall_timing ? (report.overall_timing as Record<string, string>)['avg_total'] : 'n/a'}  min: ${report.overall_timing ? (report.overall_timing as Record<string, string>)['min_total'] : 'n/a'}  max: ${report.overall_timing ? (report.overall_timing as Record<string, string>)['max_total'] : 'n/a'}  p95: ${report.overall_timing ? (report.overall_timing as Record<string, string>)['p95_total'] : 'n/a'}`,
      '',
...
server.registerTool(
  ...
  async ({ workflow_content }) => {
    ...
    const hasYarnInstall = hasPattern(/yarn\s+(?:install|)/);
    ...
    // --- Checkout depth ---
    const hasShallowClone = hasPattern(/fetch-depth:\s*[1-9]/);
    ...
    for (const severity of ['critical', 'warning', 'info', 'good'] as const) {
      ...
      const icon = severity === 'good' ? '[OK]' : severity === 'info' ? '[INFO]' : severity === 'warning' ? '[WARN]' : '[CRIT]';
  ...
  async ({ owner, repo }) => {
    ...
    if (billing) {
      ...
      for (const [type, minutes] of Object.entries(breakdown)) {
        ...
        estimatedCost += cost;
        breakdownLines.push(`  ${type.padEnd(20)} ${String(minutes).padStart(8)} min   $${cost.toFixed(2)}`);
      }
      ...
      const percentUsed = includedMins > 0 ? ((usedMins / includedMins) * 100).toFixed(1) : 'N/A';
    ...
    if (repo) {
      try {
        ...
        const cacheUsedPct = ((cache.active_caches_size_in_bytes / (10 * 1_073_741_824)) * 100).toFixed(1);
      ...
      try {
        ...
        for (const run of recentRuns) {
          totalSeconds += durationSeconds(run.run_started_at, run.updated_at);
        }

        const totalMinutes = totalSeconds / 60;
        const estimatedUbuntuCost = totalMinutes * 0.008;

        resultSections.push(
          '',
          `Recent Run Timing: ${owner}/${repo} (last ${recentRuns.length} completed runs)`,
          `  Approximate total wall-clock time: ${fmtSeconds(totalSeconds)}`,
          `  Approximate total minutes:         ${totalMinutes.toFixed(1)} min`,
          `  Estimated cost (Ubuntu rate):      $${estimatedUbuntuCost.toFixed(2)}`,
          '',
          'Note: Wall-clock time is not the same as billable minutes when jobs run in parallel.'
        );

        structuredResult['recent_runs_estimate'] = {
          runs_counted: recentRuns.length,
          total_wall_clock_seconds: parseFloat(totalSeconds.toFixed(1)),
          total_minutes_approx: parseFloat(totalMinutes.toFixed(1)),
